...
public abstract class Enemy {
    ...
    //Image
    private int solidWidth = 24;
    private int solidHeight = 40;
    ...
    public Enemy(GamePanel gamePanel, Point[] waypoints, boolean isGhost, String name) {
        ...
        if (speedCount >= speed){
            if (x != targetX) {
                if (x < targetX) {
                    ...
                    x = Math.max(x - 1, targetX);
                    direction = "left";
                }
            } else if (y != targetY) {
                if (y < targetY) {
                    y = Math.min(y + 1, targetY);
                    direction = "down";
                } else {
                    y = Math.max(y - 1, targetY);
                    direction = "up";
                }
            } else {
